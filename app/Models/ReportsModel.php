...
class ReportsModel extends Model
{
    ...
    public function get_office_name($params){
        $builder = $this->db->table('tbloffice');
        $builder->where('officeid',$params['officeid']);
        $query = $builder->get();

        return $query->getRowArray();
    }

    public function get_reponses($params){
        ...
        // Return the result as an array of objects or any other format you prefer
        return $query->getResultArray();
    }
    ...
    public function gen_sex($params){
        ...
        $builder = $this->db->table('tblcss_summary csssum')
            ->select([
                'SUM(CASE WHEN sex = \'Male\' AND ser.is_external = 1 THEN 1 ELSE 0 END) AS external_male',
                'SUM(CASE WHEN sex = \'Female\' AND ser.is_external = 1 THEN 1 ELSE 0 END) AS external_female',
                'SUM(CASE WHEN ser.is_external = 1 THEN 1 ELSE 0 END) AS total_external',
                'SUM(CASE WHEN sex = \'Male\' AND ser.is_external = 0 THEN 1 ELSE 0 END) AS internal_male',
                'SUM(CASE WHEN sex = \'Female\' AND ser.is_external = 0 THEN 1 ELSE 0 END) AS internal_female',
                'SUM(CASE WHEN ser.is_external = 0 THEN 1 ELSE 0 END) AS total_internal'
            ])
            ->join('tblservices ser', 'ser.servicesid = csssum.servicesid')
            ->join('tblquarters quar', 'quar.quarterid = csssum.quarterid')
            ->join('tblsemester sem', 'sem.semesterid = quar.semesterid')
    ...
    public function gen_age_distribution($params){
        ...
        if ($params['officeid'] != 'all') {
        $temp_table .= ' AND csssum.officeid = ' . $params['officeid'];
        }


        if ($params['typeselector'] === 'semester') {
            if ($params['semesterid'] != 'all') {
                $temp_table .= ' AND quar.semesterid = '.$params['semesterid'];
            }
        }

        if ($params['typeselector'] === 'quarter') {
            $temp_table .= ' AND quar.quarterid = '.$params['quarterid'];
        }    
        ...
        if ($params['officeid'] != 'all') {
        $temp_table_total .= ' AND csssum.officeid = ' . $params['officeid'];
        }


        if ($params['typeselector'] === 'semester') {
            if ($params['semesterid'] != 'all') {
                $temp_table_total .= ' AND quar.semesterid ='.$params['semesterid'];
            }
        }

        if ($params['typeselector'] === 'quarter') {
            $temp_table_total .= ' AND quar.quarterid ='.$params['quarterid'];
        }
    ...
    public function gen_vulsector($params){
        $builder = $this->db->table('tblcss_summary as csssum')
        ->select('
        ...
        ')
        ->join('tblquarters quar', 'quar.quarterid = csssum.quarterid', 'left')
        ->join('tblservices ser', 'ser.servicesid = csssum.servicesid', 'left')
        ->where('csssum.year', $params['year']);
    ...
    public function gen_cc($params){
        ...
        ->join('tblcss_summary csssum', 'csssum.csssummaryid = cc.csssummaryid')
        ->join('tblquarters quar', 'quar.quarterid = csssum.quarterid')
        ->join('tblservices ser', 'ser.servicesid = csssum.servicesid')
        ->where('csssum.year', $params['year']);
    ...
    public function gen_sqd($params){
        $builder = $this->db->table('tblcss_details_sqd sqd')
            ...
            ->join('tblcss_summary csssum', 'csssum.csssummaryid = sqd.csssummaryid')
            ->join('tblquarters quar', 'quar.quarterid = csssum.quarterid')
            ->join('tblservices ser', 'ser.servicesid = csssum.servicesid')
            ->where('csssum.year', $params['year']);
